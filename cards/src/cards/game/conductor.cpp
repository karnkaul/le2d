...
Conductor::Conductor(gsl::not_null<IServices const*> services, gsl::not_null<Board*> board)
	: m_services(services), m_timings(&services->get_catalog().get_config().timings), m_board(board) {}
...
void Conductor::start_game(GameInfo const& game_info) {
	...
	m_board->play_state = {};
	m_board->play_state.tally.custom_pointer = m_game_info.custom_pointer;
...
void Conductor::tick_deal(kvf::Seconds const dt) {
	...
	m_board->get_pawn(deal.next_seat).add_card(deal.denominations.back(), face);
	...
	if (!deal.sfx_played) {
		deal.sfx_played = true;
		play_sfx_if(m_services->get_catalog().get_distribute_sfx());
	}
}

void Conductor::tick_wait_submit([[maybe_unused]] kvf::Seconds dt) {
	auto card = m_board->select_submit();
	if (!card) { return; }
...
void Conductor::tick_discard(kvf::Seconds const dt) {
	...
	if (!m_discard.sfx_played) {
		m_discard.sfx_played = true;
		play_sfx_if(m_services->get_catalog().get_discard_sfx());
	}
...
void Conductor::shared_tick(kvf::Seconds const dt) {
	...
	if (ImGui::Begin("Conductor")) {
		...
		if (ImGui::TreeNode("tally")) {
			for (auto const seat : util::all_seats_v) {
				auto const points = m_board->play_state.tally.points.at(seat);
...
void Conductor::finish_submit() {
	...
	m_board->play_state.trick_state = m_board->trick.get_state();

	play_sfx_if(m_services->get_catalog().get_submit_sfx());
...
void Conductor::start_discard() {
	...
	auto const winner = evaluator.evaluate_winner(m_board->play_state);
	...
	m_board->play_state.tally.triage(discard.round);
...
void Conductor::finish_discard() {
	...
	for (auto& card : discard.round.cards.values) { m_board->discarded.push_back(std::move(card)); }
	m_board->current_seat = discard.round.winner;
	m_board->play_state.trick_state = {};
	m_anim = std::monostate{};

	if (m_board->are_hands_empty()) {
		m_state = State::Idle;
		return;
	}
...
void Conductor::play_sfx_if(klib::Ptr<le::IAudioBuffer const> buffer) const {
	if (!buffer) { return; }
	m_services->get_context().get_audio_mixer().play_sfx(buffer);
}
...
void Conductor::Discard::draw(le::IRenderer& renderer) const {
	...
	for (auto i = 0; i < 4; ++i) {
		round.cards.at(seat).draw(renderer);
