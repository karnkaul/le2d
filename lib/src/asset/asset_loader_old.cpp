#include <le2d/asset/asset_loader_old.hpp>
#include <le2d/json_io.hpp>
#include <log.hpp>

namespace le {
AssetLoaderOld::AssetLoaderOld(gsl::not_null<IDataLoader const*> data_loader, gsl::not_null<IResourceFactory const*> resource_factory)
	: m_data_loader(data_loader), m_resource_factory(resource_factory) {}

void AssetLoaderOld::on_success(std::string_view const type, std::string_view const uri) { log.info("=='{}'== {} loaded", uri, type); }

void AssetLoaderOld::on_failure(std::string_view const type, std::string_view const uri) { log.warn("'{}' Failed to load '{}'", type, uri); }

auto AssetLoaderOld::load_tile_set(std::string_view const uri) const -> std::unique_ptr<TileSet> {
...
auto AssetLoaderOld::load_transform_animation(std::string_view const uri) const -> std::unique_ptr<TransformAnimation> {
...
auto AssetLoaderOld::load_flipbook_animation(std::string_view const uri) const -> std::unique_ptr<FlipbookAnimation> {
...
auto AssetLoaderOld::load_shader(std::string_view const vertex_uri, std::string_view const fragment_uri) const -> std::unique_ptr<IShader> {
...
auto AssetLoaderOld::load_font(std::string_view const uri) const -> std::unique_ptr<IFont> {
...
auto AssetLoaderOld::load_texture(std::string_view const uri) const -> std::unique_ptr<ITexture> {
...
auto AssetLoaderOld::load_tile_sheet(std::string_view const uri, std::string* out_texture_uri) const -> std::unique_ptr<ITileSheet> {
...
auto AssetLoaderOld::load_audio_buffer(std::string_view const uri) const -> std::unique_ptr<IAudioBuffer> {
