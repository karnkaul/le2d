...
class ResourceFactory : public IResourceFactory {
  ...
  private:
	...
	[[nodiscard]] auto create_tilesheet(TextureSampler const& sampler) const -> std::unique_ptr<ITileSheet> final {
		return std::make_unique<TileSheet>(m_render_device, m_sampler_factory, sampler);
	}

	[[nodiscard]] auto create_font() const -> std::unique_ptr<IFont> final { return std::make_unique<Font>(m_render_device, m_sampler_factory); }
