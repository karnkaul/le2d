...
RenderPass::RenderPass(gsl::not_null<kvf::RenderDevice*> render_device, gsl::not_null<ResourcePool*> resource_pool, vk::SampleCountFlagBits samples)
	: m_render_device(render_device), m_resource_pool(resource_pool), m_render_pass(render_device, samples), m_blocker(render_device->get_device()) {
...
auto RenderPass::begin_render(vk::CommandBuffer command_buffer, glm::ivec2 size) -> Renderer {
	size = clamp_size(size);
	m_render_pass.begin_render(command_buffer, kvf::util::to_vk_extent(size));
	return Renderer{*this, command_buffer};
}
} // namespace le
