...
void Sprite::set_texture(ITexture const* texture, kvf::UvRect const& uv) {
	if (texture == m_quad.texture && uv == get_uv()) { return; }
...
void Sprite::update(glm::vec2 const base_size, glm::vec2 const origin, kvf::UvRect const& uv) {
	auto const size = [&] {
		if (m_quad.texture == nullptr || m_aspect == kvf::ResizeAspect::None) { return base_size; }
